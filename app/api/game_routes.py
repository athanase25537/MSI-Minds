...
from ..models.schemas import NewGameRequest, MoveRequest
from ..api.game_services import GameService
from ..api.deps import get_game_service

router = APIRouter()

@router.post("/game/new", response_model=dict)
async def new_game(request: NewGameRequest, service: GameService = Depends(get_game_service)):
    game_id = service.create_game(request)
    return {"game_id": game_id}

@router.get("/game/{game_id}/state", response_model=dict)
async def get_state(game_id: str, service: GameService = Depends(get_game_service)):
    try:
...
async def ai_move(game_id: str, service: GameService = Depends(get_game_service)):
    try:
...
async def reset_game(game_id: str, service: GameService = Depends(get_game_service)):
    try:
